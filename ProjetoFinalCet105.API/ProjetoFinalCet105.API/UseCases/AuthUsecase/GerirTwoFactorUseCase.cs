...
namespace ProjetoFinalCet105.API.UseCases.AuthUsecase
{
    public class GerirTwoFactorUseCase
    {
        ...
        public async Task<UseCaseResult<bool>> GetEstadoAsync(string userId)
        {
            ...
            return UseCaseResult<bool>.Ok(ativo);
        }

        public async Task<UseCaseResult<bool>> ExecuteAsync(string userId, bool ativo)
        {
            var user = await _userManager.FindByIdAsync(userId);
